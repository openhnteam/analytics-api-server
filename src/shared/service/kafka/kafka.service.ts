...
export class KafkaService implements OnModuleInit, OnModuleDestroy {
  ...
  async stopConsumer(topic: string): Promise<boolean> {
    try {
      const consumer: Consumer = this.consumerMap.get(topic);
      if (consumer) {
        await consumer.stop();
      }
      return true;
      ...
      this.logger.error(
        "【kafka stop Consumer error】" + error + "【topic】" + topic
      );
      return false;
    }
  }

  async pauseConsumer(topic: string, partition: number): Promise<boolean> {
    try {
      const consumer: Consumer = this.consumerMap.get(topic);
      if (consumer) {
      ...
      this.logger.error(
        "【kafka pause Consumer error】" + error + "【topic】" + topic
      );
      return false;
    }
  }

  async resumeConsumer(topic: string, partition: number): Promise<boolean> {
    try {
      const consumer: Consumer = this.consumerMap.get(topic);
      if (consumer) {
      ...
      this.logger.error(
        "【kafka resume Consumer error】" + error + "【topic】" + topic
      );
    }
    return false;
  }
  ...
  async commitOffsets(
    ...
    try {
      ...
      this.logger.error(
        "【kafka commitOffsets Consumer error】" + error + "【topic】" + topic
      );
      return false;
    }
  }
